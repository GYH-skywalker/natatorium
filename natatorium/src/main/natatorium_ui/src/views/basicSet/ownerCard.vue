  ...
  <div>
    <operate-panel @search="search" hint="项目名称">
      <template #button>
        ...
        <el-button type="primary" @click="resetPackage">
          重设套餐
        </el-button>
      </template>
    </operate-panel>
...
<!--        </el-table-column>-->
        ...
        <el-table-column
            ...
            width="267">
          <template slot-scope="scope">
            <el-button @click="del(scope.row.id)" size="mini" :disabled="scope.row.enable === 0">编辑</el-button>
            <el-button @click="del(scope.row.id)" size="mini" :disabled="scope.row.enable === 0">全部办理</el-button>
            <el-button @click="del(scope.row.id)" size="mini" :disabled="scope.row.enable === 0">新增业主办理</el-button>
            <el-button @click="del(scope.row.id)" size="mini" :disabled="scope.row.enable === 0">删除</el-button>
          </template>
        </el-table-column>
      </template>
    </nata-table>
    <nata-dialog @confirm="confirm" title="新增套餐" width="45%">
      <el-form
          label-position="left"
          label-width="75px"
          title="新增套餐"
  ...
  data(){
    return{
      fields: [
        ...
        {prop:'packageYear',label:'套餐年份',width:80},
        {prop:'startTime',label:'开始日期',width:90},
        {prop:'endTime',label:'结束日期',width:90},
