<template>
  <div>
    <operate-panel @search="search" hint="班次名称">
      <template #button>
    ...
    <nata-table
      ...
      <template #customColumn>
        ...
        <el-table-column
            ...
            width="80">
          <template slot-scope="scope">
            <el-button @click="editTimeRange(scope.row.id)" size="mini" :disabled="scope.row.enable === 0">编辑时间段</el-button>
          </template>
        </el-table-column>
        <el-table-column
            fixed="right"
            ...
            width="80">
          <template slot-scope="scope">
            <el-button @click="editPeopleQuantity(scope.row.id)" size="mini" :disabled="scope.row.enable === 0">编辑人数</el-button>
          </template>
        </el-table-column>
        <el-table-column
            fixed="right"
    ...
    <nata-dialog @confirm="confirm" title="新增培训班" width="45%">
      <el-form
          label-position="left"
          label-width="95px"
          title="用户信息"
        ...
        <el-form-item label="提醒课时">
          <el-input v-model="Form.warnPeriod"></el-input>
        </el-form-item>
        <el-form-item label="允许时差(分)">
          <el-input v-model="Form.permitDeltaT"></el-input>
        </el-form-item>
...
export default {
  ...
  data(){
    return{
      fields: [
        {prop:'expireTime',label:'过期时间',width:120},
        {prop:'packagePrice',label:'套餐价格',width:80},
        {prop:'permitDeltaT',label:'允许时间差',width:80},
        {prop:'period',label:'课时',width:80},
        {prop:'warnPeriod',label:'提醒课时',width:80},
        {prop:'population',label:'备注',width:80},
        {prop:'coach',label:'教练',width:80},
