<template>
  <div>
    <operate-panel @search="search" hint="项目名称">
      <template #input>
        <el-select v-model="sCondition.cardType" @change="typeChange" placeholder="请选择">
          <el-option
...
export default {
  ...
  data(){
    return{
      fields: [
        {prop:'packageName',label:'教练姓名',width:80},
        {prop:'packageCounts',label:'套餐名称',width:80},
        {prop:'packagePrice',label:'消费时间',width:120},
        {prop:'startTime',label:'会员名称',width:90},
        {prop:'endTime',label:'教练评语',width:90},
        {prop:'warnTimes',label:'评价时间(教练)',width:120},
        {prop:'cardType',label:'学员评语',width:80},
        {prop:'warnTimes',label:'评价时间(学员)',width:120},
        {prop:'warnTimes',label:'评价进度',width:80},
